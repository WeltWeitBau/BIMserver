...
import org.bimserver.ifc.step.deserializer.IfcStepStreamingDeserializer;
...
import de.weiltweitbau.deserializers.IfcParserWriterUtils.ConvertSimpleValueException;
...
public class WwbIfcStepStreamingDeserializer implements StreamingDeserializer {
	...
	// This is enabled for now so we can test more models, not decided yet whether
	// this will be enabled in the final release
	...
	@Override
	public long read(InputStream in, String filename, long fileSize, QueryContext reusable)
		...
		if (filename != null
			...
			try {
				...
				if (nextEntry == null) {
					throw new DeserializeException(DeserializerErrorCode.IFCZIP_CONTAINS_NO_IFC_FILES,
				...
				if (nextEntry.getName().toUpperCase().endsWith(".IFC")) {
					...
					if (size == 0) {
						throw new DeserializeException(DeserializerErrorCode.IFCZIP_CONTAINS_EMPTY_IFC_MODEL,
	...
	private long read(InputStream inputStream, long fileSize) throws DeserializeException {
		...
		try {
			...
			if (line == null) {
				throw new DeserializeException(DeserializerErrorCode.UNEXPECTED_END_OF_STREAM_WHILE_READING_FIRST_LINE,
						0, "Unexpected end of stream reading first line");
			}
...
//			model.getModelMetaData().setChecksum(md.digest());
			if (mode == Mode.HEADER) {
				throw new DeserializeException(DeserializerErrorCode.NO_VALID_IFC_HEADER_FOUND, lineNumber,
						"No valid IFC header found");
			}

			addErrors(waitingList.getErrors());
	...
	private void addErrors(Map<Long, String> errors) {
		for (long expressId : errors.keySet()) {
			writeErrorLog(expressId, errors.get(expressId));
		}
	}
	...
	private boolean processLine(String line)
		...
		try {
			...
			// added by WWB
			throw new DeserializeException(e.getDeserializerErrorCode(), e.getLineNumber(),
					getProcessLineErrorMessage(e, line), e);
		}
	...
	private void processHeader(String line) throws DeserializeException {
		...
		if (line.startsWith("FILE_DESCRIPTION")) {
			...
			if (!ifcSchemaVersion.toLowerCase().equalsIgnoreCase(schema.getHeaderName().toLowerCase())) {
				throw new DeserializeException(DeserializerErrorCode.IFC_SCHEMA_NOT_SUPPORTED_BY_DESERIALIZER,
						lineNumber, ifcSchemaVersion + " is not supported by this deserializer ("
								+ schema.getHeaderName() + " is)");
			}
	...
	public void processRecord(String line) throws DeserializeException, MetaDataException, BimserverDatabaseException {
		...
		if (lastIndexOfSemiColon == -1) {
			throw new DeserializeException(DeserializerErrorCode.NO_SEMICOLON_FOUND_IN_RECORD, lineNumber,
					"No semicolon found in line");
		}
		int indexOfFirstParen = line.indexOf("(", equalSignLocation);
		if (indexOfFirstParen == -1) {
			throw new DeserializeException(DeserializerErrorCode.NO_LEFT_PARENTHESIS_FOUND_IN_RECORD, lineNumber,
					"No left parenthesis found in line");
		}
		...
		if (eClass == null) {
			throw new DeserializeException(DeserializerErrorCode.UNKNOWN_ENTITY, lineNumber,
				...
				if (getPackageMetaData().useForSerialization(eClass, eStructuralFeature)) {
					if (getPackageMetaData().useForDatabaseStorage(eClass, eStructuralFeature)) {
						...
						try {
							...
							throw new DeserializeException(DeserializerErrorCode.NOT_ENOUGH_FIELDS_FOR_ENTITY,
									lineNumber, eClass.getName() + " expects " + expected + " fields, but less found ("
											+ e.getMessage() + ")",
									e);
						}
						lastIndex = nextIndex;
						if (val.length() == 0) {
							throw new DeserializeException(DeserializerErrorCode.EXPECTED_CHARACTER_BUT_EMPTY_FIELD,
									lineNumber, "Expected non-comma character, but field value length is 0");
						}
						char firstChar = val.charAt(0);
						if (firstChar == '$') {
							object.eUnset(eStructuralFeature);
							if (eStructuralFeature.getEType() == EcorePackage.eINSTANCE.getEDouble()) {
								EStructuralFeature doubleStringFeature = eClass
							...
							if (!eStructuralFeature.isMany()) {
								...
								if (eStructuralFeature.getName().equals("GlobalId")) {
									...
									if (eStructuralFeature instanceof EAttribute) {
										...
										object.setReference((EReference) eStructuralFeature,
												(WrappedVirtualObject) converted);
									}
								}
								if (eStructuralFeature.getEType() == EcorePackage.eINSTANCE.getEDouble()) {
									EStructuralFeature doubleStringFeature = eClass
					...
					if (getPackageMetaData().useForDatabaseStorage(eClass, eStructuralFeature)) {
						if (eStructuralFeature instanceof EReference
								&& getPackageMetaData().isInverse((EReference) eStructuralFeature)) {
							object.eUnset(eStructuralFeature);
						} else {
	...
	private void processGuid(VirtualObject object, EStructuralFeature eStructuralFeature, Object converted)
			throws BimserverDatabaseException, DeserializeException {
		try {
			...
			if (CONVERT_INVALID_IFC_GUIDS) {
				// The GUID is invalid according to the IFC spec, sometimes another
				// representation is used in IFC files, which is a valid GUID, but not according
				// to the IFC spec
				// The next bit of code tries this variant, it remains to be decided what will
				// be the final implementation for this in deserializer
				try {
					UUID uuid = UUID.fromString(converted.toString());
					String ifcGuid = GuidCompressor.compressGuidString(uuid.toString());
					LOGGER.warn("Invalid GUID on line " + lineNumber + " , converted from default UUID format ("
							+ uuid.toString() + ") to IFC format (" + ifcGuid + ")");
					// If this succeeds, we convert the UUID to the IFC format, and store that, so
					...
					throw new DeserializeException(DeserializerErrorCode.INVALID_GUID, lineNumber,
							"Invalid GUID: \"" + converted.toString() + "\": " + e.getMessage());
				}
			} else {
				throw new DeserializeException(DeserializerErrorCode.INVALID_GUID, lineNumber,
						"Invalid GUID: \"" + converted.toString() + "\": " + e.getMessage());
	...
	private boolean readList(String val, ListCapableVirtualObject object, EStructuralFeature structuralFeature,
			VirtualObject parentObject, int parentIndex)
			throws DeserializeException, MetaDataException, BimserverDatabaseException {
		int index = 0;
		if (!structuralFeature.isMany()) {
			throw new DeserializeException(DeserializerErrorCode.UNEXPECTED_AGGREGATION, lineNumber,
					"Field " + structuralFeature.getName() + " of "
							+ (parentObject == null ? structuralFeature.getEContainingClass().getName()
		...
		if (isDouble) {
			doubleStringFeature = structuralFeature.getEContainingClass()
			...
			if (stringValue.length() > 0) {
				if (stringValue.charAt(0) == '#') {
					...
					if (mappedObjects.containsKey(referenceId)) {
						...
						if (referencedOid != null) {
							...
							if (((EClass) structuralFeature.getEType()).isSuperTypeOf(referenceEClass)) {
								...
								object.setListItemReference(structuralFeature, index, referenceEClass, referencedOid,
										-1);
							} else {
								throw new DeserializeException(
										DeserializerErrorCode.REFERENCED_OBJECT_CANNOT_BE_STORED_IN_THIS_FIELD,
										lineNumber, referenceEClass.getName() + " cannot be stored in "
						...
						if (object instanceof VirtualObject) {
							WaitingVirtualObject waitingObject = new ListWaitingVirtualObject(lineNumber,
									(VirtualObject) object, structuralFeature, index, pos);
							waitingObject.setExpressId(expressID);

							waitingList.add(referenceId, waitingObject);
						} else if (object instanceof ByteBufferList) {
							if (parentObject == null) {
								throw new DeserializeException(DeserializerErrorCode.INTERNAL_BIMSERVER_ERROR,
										lineNumber, "Need a parentObject");
							}

							WaitingVirtualObject waitingObject = new TwoDimensionalListWaitingVirtualObject(lineNumber,
									parentObject, (ByteBufferList) object, structuralFeature, parentIndex, index, pos);
							waitingObject.setExpressId(expressID);
					...
					if (structuralFeature.getEType() instanceof EClass) {
						...
						if (!readList(stringValue, newObject, newObject.eClass().getEStructuralFeature("List"),
								parentObject, index)) {
							complete = false;
						}
						object.setListItem(structuralFeature, index, newObject);
					} else {
						PrimitiveByteBufferList newObject = new PrimitiveByteBufferList(reusable,
								(EDataType) structuralFeature.getEType());
	...
	private Object convert(EStructuralFeature eStructuralFeature, EClassifier classifier, String value)
		...
		if (classifier != null) {
			if (classifier instanceof EClassImpl) {
						...
						if (value.equals("")) {
							...
							if (instanceClass == Integer.class || instanceClass == int.class) {
								try {
									...
									throw new DeserializeException(DeserializerErrorCode.INVALID_INTEGER_LITERAL,
											lineNumber, value + " is not a valid integer value");
								}
								...
								try {
									...
									throw new DeserializeException(DeserializerErrorCode.INVALID_DOUBLE_LITERAL,
											lineNumber, value + " is not a valid double floating point number");
								}
								newObject.setAttribute((EAttribute) newObject.eClass()
										.getEStructuralFeature(WRAPPED_VALUE + "AsString"), value);
							} else if (instanceClass == String.class) {
								newObject.setAttribute(wrappedFeature,
										IfcParserWriterUtils.readString(value, lineNumber));
								...
								EEnumLiteral enumValue = (((EEnumImpl) feature.getEType())
										.getEEnumLiteral(realEnumValue));
								if (enumValue == null) {
									// added by WWB
									if (feature.isRequired() == false) {
										StringBuilder message = new StringBuilder(255);
										message.append("Enum type ").append(feature.getEType().getName())
												.append(" has no literal value '").append(realEnumValue).append("'");

										writeErrorLog(expressID, message.toString());
										return newObject;
									}

									throw new DeserializeException(DeserializerErrorCode.NON_EXISTING_ENUM_LITERAL_USED,
											lineNumber, "Enum type " + feature.getEType().getName()
													+ " has no literal value '" + realEnumValue + "'");
								}
								newObject.setAttribute(wrappedFeature, enumValue);
							} else {
								throw new DeserializeException(DeserializerErrorCode.UNIMPLEMENTED_BIMSERVER_FEATURE,
										lineNumber, "Not implemented: " + instanceClass);
							}
						}
						return newObject;
					}
				...
				try {
					return IfcParserWriterUtils.convertSimpleValue(getPackageMetaData(), eStructuralFeature,
							classifier.getInstanceClass(), value, lineNumber);
				} catch (ConvertSimpleValueException e) {
					writeErrorLog(expressID, e.getMessage());
					return null;
				}
			}
		}
	...
	private Object processInline(EStructuralFeature eStructuralFeature, String value)
			throws DeserializeException, MetaDataException, BimserverDatabaseException {
		if (value.indexOf("(") != -1) {
			...
			if (eClassifier instanceof EClass) {
				...
				throw new DeserializeException(DeserializerErrorCode.UNKNOWN_ENTITY, lineNumber,
						typeName + " is not an existing IFC entity");
			}
		} else {
			try {
				return IfcParserWriterUtils.convertSimpleValue(getPackageMetaData(), eStructuralFeature,
						eStructuralFeature.getEType().getInstanceClass(), value, lineNumber);
			} catch (ConvertSimpleValueException e) {
				writeErrorLog(expressID, e.getMessage());
			}
	...
	private void readEnum(String val, VirtualObject object, EAttribute structuralFeature)
			throws DeserializeException, MetaDataException, BimserverDatabaseException {
		if (val.equals(".T.")) {
			if (structuralFeature.getEType().getName().equals("Tristate")) {
				object.setAttribute(structuralFeature,
						getPackageMetaData().getEEnumLiteral("Tristate", "TRUE").getInstance());
				...
				createIfcBoolean.setAttribute((EAttribute) eClass.getEStructuralFeature("WrappedValue"),
						getPackageMetaData().getEEnumLiteral("Tristate", "TRUE").getInstance());
			...
			if (structuralFeature.getEType().getName().equals("Tristate")) {
				object.setAttribute(structuralFeature,
						getPackageMetaData().getEEnumLiteral("Tristate", "FALSE").getInstance());
			} else if (structuralFeature.getEType().getName().equals("IfcBoolean")) {
				EClass eClass = getPackageMetaData().getEClass("IfcBoolean");
				VirtualObject createIfcBoolean = newVirtualObject(eClass, val.length());
				createIfcBoolean.setAttribute((EAttribute) eClass.getEStructuralFeature("WrappedValue"),
			...
			if (structuralFeature.getEType().getName().equals("Tristate")) {
				object.setAttribute(structuralFeature,
						getPackageMetaData().getEEnumLiteral("Tristate", "UNDEFINED").getInstance());
			} else if (structuralFeature.getEType() == EcorePackage.eINSTANCE.getEBoolean()) {
			...
			if (structuralFeature.getEType() instanceof EEnumImpl) {
				...
				if (enumValue == null) {
					...
					if (structuralFeature.getEType() == Ifc4Package.eINSTANCE.getIfcExternalSpatialElementTypeEnum()
							&& realEnumValue.equals("NOTDEFIEND")) {
				...
				if (enumValue == null) {
					// added by WWB
					if (structuralFeature.isRequired() == false) {
						StringBuilder message = new StringBuilder(255);
						message.append("Enum type ").append(structuralFeature.getEType().getName())
								.append(" has no literal value '").append(realEnumValue)
								.append("'. Unsetting Attribute '").append(structuralFeature.getName()).append("'.");
					...
					throw new DeserializeException(DeserializerErrorCode.NON_EXISTING_ENUM_LITERAL_USED, lineNumber,
							"Enum type " + structuralFeature.getEType().getName() + " has no literal value '"
									+ realEnumValue + "'");
				}
				object.setAttribute(structuralFeature, enumValue.getInstance());
			} else {
				throw new DeserializeException(DeserializerErrorCode.UNEXPECTED_ENUM, lineNumber, "Value " + val
						+ " indicates enum type but " + structuralFeature.getEType().getName() + " expected");
			}
		}
	}

	private boolean readReference(String val, VirtualObject object, EStructuralFeature structuralFeature)
			throws DeserializeException, BimserverDatabaseException {
		if (structuralFeature == Ifc4Package.eINSTANCE.getIfcIndexedColourMap_Opacity()) {
			...
			object.setAttribute((EAttribute) structuralFeature.getEContainingClass()
					.getEStructuralFeature(structuralFeature.getName() + "AsString"), "0");
			return true;
		}
		try {
			...
			if (mappedObjects.containsKey(referenceId)) {
				...
				WaitingVirtualObject waitingObject = new SingleWaitingVirtualObject(lineNumber, object,
						structuralFeature, pos);
				waitingObject.setExpressId(expressID);
				waitingList.add(referenceId, waitingObject);
				return false;
			}
		} catch (NumberFormatException e) {
			throw new DeserializeException(DeserializerErrorCode.INVALID_REFERENCE, lineNumber,
					"'" + val + "' is not a valid reference");
		}
	}

	private void writeErrorLog(long expressId, String message) {
		errors.put(expressId, message);
	}
